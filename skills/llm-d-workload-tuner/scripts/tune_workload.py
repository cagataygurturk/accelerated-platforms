...
def parse_args():
    ...
    parser.add_argument(
        "--gpu-type",
        required=True,
        help="Accelerator type (e.g. rtx-pro-6000, nvidia-h100, v6e)",
    )
    parser.add_argument(
        "--strategy",
...
def main():
    ...
    weights_size_gb = (
        model_spec["parameters"] * 2.0
    ) * 1.2  # Including overhead margins

    # Cache per token (in bytes)
    # Cache = 2 * Layers * KV_Heads * Head_Dim * 2 bytes (BF16)
    bytes_per_token = (
        2 * model_spec["layers"] * model_spec["kv_heads"] * model_spec["head_dim"] * 2
    )
    ...
    while tp_size * gpu_vram < total_required_gb:
        if tp_size < 4:
            ...
            print(
                "\n[Error] Workload exceeds capacity limits even with FP8 quantization.",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    available_cache_gb = (tp_size * gpu_vram) - (
        weights_size_gb
        if quantization == "null"
        else (model_spec["parameters"] * 1.0 * 1.2)
    )
    max_safe_len = int(
    ...
    if args.apply:
        ...
        accel_dir = (
            ...
            else f"{args.gpu_type.lower()}-{model_suffix}"
        )
        ...
        if not os.path.isdir(overlay_path):
            print(
                f"Error: Target overlay directory not found: {overlay_path}",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        if os.path.exists(resource_file) and platform_prefix == "gpu":
            ...
            try:
                limits = res_data["spec"]["template"]["spec"]["containers"][0][
                    "resources"
                ]["limits"]
                ...
                print(
                    f"Warning: Failed to parse and patch resources in {resource_file}: {e}"
                )
        ...
        if os.path.exists(nodeselector_file) and platform_prefix == "gpu":
            ...
            try:
                ...
                print(
                    f"Updated NodeSelector constraints in {nodeselector_file} to {tp_size}"
                )
            except (KeyError, TypeError) as e:
                print(
                    f"Warning: Failed to parse and patch node selectors in {nodeselector_file}: {e}"
                )
